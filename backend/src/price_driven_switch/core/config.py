...
# Get project root (4 levels up from core/config.py)
PROJECT_ROOT = CURRENT_FILE.parents[3]

# Config directory relative to project root
CONFIG_DIR = PROJECT_ROOT / "src" / "price_driven_switch" / "store"
CONFIG_DIR.mkdir(parents=True, exist_ok=True)

# Settings and env file paths
PATH_SETTINGS = CONFIG_DIR / "settings.toml"
...
logger.debug(f"PATH_SETTINGS: {PATH_SETTINGS}")
...
default_settings_toml = {
    "Appliances": {
        ...
        "Floor": {"Power": 0.8, "Priority": 3, "Setpoint": 0.5},
    },
...
def create_default_settings_if_none(
    path: Union[str, Path] = PATH_SETTINGS,
) -> None:
...
# for consistent file updates
debounce_time = 0.5  # seconds
debounce_timer = None
file_lock = threading.Lock()


def load_settings_file(path: Union[str, Path] = PATH_SETTINGS) -> dict:
    with file_lock, open(path, mode="r", encoding="utf-8") as toml_file:
        settings = toml.load(toml_file)
        validate_settings(settings)
        logger.debug(f"Loaded settings from file: {settings}")
        return settings


def read_power_limit() -> float:
    return load_settings_file().get("Settings", {}).get("MaxPower", {})
...
def save_settings(new_settings: dict, path: Union[str, Path] = PATH_SETTINGS) -> None:
...
def save_api_key(api_key: str) -> None:
    set_key(ENV_FILE, "TIBBER_TOKEN", api_key)
